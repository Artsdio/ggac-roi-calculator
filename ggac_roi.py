...
# ── PARSER EXCEL MODAL_GGAC (DIPERBAIKI: BACA ITEM DENGAN BENAR) ───────────
def parse_modal_excel(file_bytes):
    """
    Baca Modal_GGAC_v3.xlsx — struktur v3 dengan ringkasan di baris 31-36.
    Fase 1: baris 6-9 (4 item), Fase 2: baris 14-18 (5 item), Penunjang: baris 22-27.
    """
    ...
    if "Investasi Investor" in wb.sheetnames:
        ...
        # ── Membaca dari RINGKASAN PERHITUNGAN (Baris 31-36) ─────────────
        gym_f1   = val("B31")  # Peralatan gym Fase 1
        gym_f2   = val("C32")  # Peralatan gym Fase 2
        pen      = val("B33")  # Fasilitas penunjang
        buf_f1   = val("B35")  # Buffer Fase 1
        buf_f12  = val("C35")  # Buffer Fase 1+2
        total_f1 = val("B36")  # TOTAL INVESTASI OWNER FASE 1
        total_f12= val("C36")  # TOTAL INVESTASI OWNER FASE 1+2
        
        # Buffer percentage dari sel E35
        buf_pct_cell = ws2["E35"].value
        buf_pct = float(buf_pct_cell) if isinstance(buf_pct_cell, (int, float)) else 0.10
        ...
        # ── Membaca item detail (DIPERBAIKI: baris yang benar) ────────────
        def read_items(r_start, r_end):
            """Baca item dari baris r_start sampai r_end, filter header/subtotal"""
            rows = []
            idx = 1  # Penomoran mulai dari 1
            for r in range(r_start, r_end + 1):
                nama = ws2.cell(r, 2).value
                # Skip jika nama kosong, mengandung 'Subtotal', atau 'Nama Peralatan'
                if not nama or not isinstance(nama, str):
                    continue
                nama_str = str(nama).strip()
                if 'subtotal' in nama_str.lower() or 'nama peralatan' in nama_str.lower():
                    continue
        ...
        # Fase 1: baris 6-9 (4 item)
        # Fase 2: baris 14-18 (5 item)
        # Penunjang: baris 22-27 (6 item, tapi ada yang Qty=0)
        fase1_gym  = read_items(6, 9)
        fase2_gym  = read_items(14, 18)
...
# ── FILE UPLOADER (DIPERBAIKI: TAMPILAN BREAKDOWN) ─────────────────────────
with st.expander("📂 Import dari Excel — Modal_GGAC_v3.xlsx", expanded=False):
    ...
    if uploaded is not None:
        try:
            ...
            gym_f1    = xl_data.get("sub_fase1", 0)
            gym_f2    = xl_data.get("sub_fase2", 0)
            penunjang = xl_data.get("sub_pend", 0)
            buffer_f1 = xl_data.get("buffer_f1", 0)
            buffer_f12= xl_data.get("buffer_f12", 0)
            total_f1  = xl_data.get("investasi_f1", 0)
            total_f12 = xl_data.get("investasi_f12", 0)
            ...
            with col_s1:
                st.markdown(f"""
                 <div style="background:#E8F5EE;border-radius:10px;padding:.75rem 1rem;border-left:4px solid #0F6E56">
                   <div style="font-size:11px;color:#888780;font-weight:600;text-transform:uppercase">Investasi Fase 1 (aktif)</div>
                   <div style="font-size:18px;font-weight:700;color:#0F6E56;font-family:monospace;margin:6px 0">Rp {rp(total_f1)}</div>
                   <div style="font-size:11px;color:#888780;line-height:1.8">
                     <div style="display:flex;justify-content:space-between"><span>Peralatan gym Fase 1</span><span style="font-family:DM Mono,monospace;font-weight:600">Rp {rp(gym_f1)}</span></div>
                     <div style="display:flex;justify-content:space-between"><span>Fasilitas penunjang</span><span style="font-family:DM Mono,monospace;font-weight:600">Rp {rp(penunjang)}</span></div>
                     <div style="display:flex;justify-content:space-between;border-top:1px solid #ccc;margin:4px 0;padding-top:4px"><span>Subtotal</span><span style="font-family:DM Mono,monospace;font-weight:600">Rp {rp(gym_f1 + penunjang)}</span></div>
                     <div style="display:flex;justify-content:space-between;color:#0F6E56"><span>Buffer 10%</span><span style="font-family:DM Mono,monospace;font-weight:600">Rp {rp(buffer_f1)}</span></div>
                   </div>
                   <div style="font-size:12px;font-weight:700;color:#0F6E56;margin-top:6px;padding-top:6px;border-top:2px solid #0F6E56">TOTAL: Rp {rp(total_f1)}</div>
                 </div>""", unsafe_allow_html=True)

            with col_s2:
                st.markdown(f"""
                 <div style="background:#FDF3E3;border-radius:10px;padding:.75rem 1rem;border-left:4px solid #854F0B">
                   <div style="font-size:11px;color:#888780;font-weight:600;text-transform:uppercase">Fase 2 (pengembangan)</div>
                   <div style="font-size:18px;font-weight:700;color:#854F0B;font-family:monospace;margin:6px 0">Rp {rp(gym_f2)}</div>
                   <div style="font-size:11px;color:#888780;line-height:1.8">
                     <div style="display:flex;justify-content:space-between"><span>Gym Fase 2 (tambahan)</span><span style="font-family:DM Mono,monospace;font-weight:600">Rp {rp(gym_f2)}</span></div>
                     <div style="display:flex;justify-content:space-between;margin-top:4px"><span>Subtotal Fase 1+2</span><span style="font-family:DM Mono,monospace;font-weight:600">Rp {rp(gym_f1 + gym_f2 + penunjang)}</span></div>
                     <div style="display:flex;justify-content:space-between;color:#854F0B"><span>Buffer 10%</span><span style="font-family:DM Mono,monospace;font-weight:600">Rp {rp(buffer_f12)}</span></div>
                   </div>
                   <div style="font-size:12px;font-weight:700;color:#854F0B;margin-top:6px;padding-top:6px;border-top:2px solid #854F0B">TOTAL FASE 1+2: Rp {rp(total_f12)}</div>
                 </div>""", unsafe_allow_html=True)

            with col_s3:
                st.markdown(f"""
                 <div style="background:#EDF3FB;border-radius:10px;padding:.75rem 1rem;border-left:4px solid #185FA5">
                   <div style="font-size:11px;color:#888780;font-weight:600;text-transform:uppercase">Modal Pengelola</div>
                   <div style="font-size:18px;font-weight:700;color:#185FA5;font-family:monospace;margin:6px 0">Rp {rp(mp)}</div>
                   <div style="font-size:11px;color:#888780;line-height:1.8">
                     <div style="display:flex;justify-content:space-between"><span>Nilai buku {len(xl_data.get('peralatan',[]))} item</span></div>
                     <div style="display:flex;justify-content:space-between"><span>existing per 2026</span></div>
                   </div>
                   <div style="font-size:12px;font-weight:700;color:#185FA5;margin-top:6px;padding-top:6px;border-top:2px solid #185FA5">IN-KIND CONTRIBUTION</div>
                 </div>""", unsafe_allow_html=True)
            ...
            # ── Detail tables (DIPERBAIKI: penomoran mulai dari 1) ────────
            ...
            def show_items(tab, items, label):
                with tab:
                    if items:
                        ...
                        df["no"] = df["no"].astype(int)  # Pastikan nomor integer
                        ...
                        st.dataframe(
                            df[["no", "nama", "qty_str", "harga", "total"]].rename(columns={
                                "no": "No", "nama": "Item", "qty_str": "Qty",
            ...
            with tab_alat:
                ...
                if alat:
                    ...
                    st.dataframe(
                        df_a[["no", "nama", "thn", "harga", "nb"]].rename(columns={
                            "no": "No", "nama": "Alat", "thn": "Thn Beli",
...
with st.sidebar:
    ...
    investasi = st.number_input("Investasi owner / Pihak I (Rp)",
                                value=xl_data.get("investasi", 64_086_000),
    ...
    modal_pengelola = st.number_input("Modal pengelola / Pihak II — nilai buku alat (Rp)",
                                      value=xl_data.get("modal_pengelola", 12_631_070),
...
st.markdown("---")
st.markdown('<div class="section-header">🖨️ Cetak / Download Laporan PDF</div>', unsafe_allow_html=True)
    ...
    with st.spinner("Menyusun laporan PDF..."):
        try:
            pdf_params = dict(
                ...
                fase_label   = xl_data.get("fase_label", "Fase 1" if xl_data else "Manual"),
