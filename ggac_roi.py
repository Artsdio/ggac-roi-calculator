...
def hitung(u_pct, growth_pct, cost_growth_pct,
    ...
    for m in range(1, months + 1):
        ...
        if bep_month is None and cum_owner[-1] >= 0:
            bep_month = m
        if bep_month_ii is None and cum_pengelola[-1] >= 0:
            bep_month_ii = m
...
def parse_modal_excel(file_bytes):
    ...
    if "Investasi Investor" in wb.sheetnames:
        ...
        # ── TAMPILAN RINGKASAN FASE 1 & FASE 2 YANG JELAS ─────────────────────────
        if uploaded is not None and xl_data:
            st.markdown("---")
            st.markdown("### 📦 **RINGKASAN INVESTASI PER FASE**")
            
            col_f1, col_f2 = st.columns(2)
            
            with col_f1:
                st.markdown("#### 🟢 **FASE 1 — PRIORITAS**")
                fase1_items = xl_data.get("fase1_gym", [])
                if fase1_items:
                    total_f1 = 0
                    for item in fase1_items:
                        if item["qty"] > 0:
                            st.markdown(f"✓ **{item['nama']}**")
                            st.caption(f"Qty: {item['qty']} × Rp {rp(item['harga'])} = Rp {rp(item['total'])}")
                            total_f1 += item["total"]
                    st.markdown(f"**Total Fase 1: Rp {rp(total_f1)}**")
                else:
                    st.warning("Tidak ada item Fase 1")
            
            with col_f2:
                st.markdown("#### 🟡 **FASE 2 — PENGEMBANGAN**")
                fase2_items = xl_data.get("fase2_gym", [])
                if fase2_items:
                    total_f2 = 0
                    for item in fase2_items:
                        if item["qty"] > 0:
                            st.markdown(f"✓ **{item['nama']}**")
                            st.caption(f"Qty: {item['qty']} × Rp {rp(item['harga'])} = Rp {rp(item['total'])}")
                            total_f2 += item["total"]
                    if total_f2 == 0:
                        st.info("Semua item Fase 2 Qty=0 (belum dibeli)")
                    else:
                        st.markdown(f"**Total Fase 2: Rp {rp(total_f2)}**")
                else:
                    st.warning("Tidak ada item Fase 2")
...
if 'xl_data' not in st.session_state:
    st.session_state.xl_data = {}
if 'file_hash' not in st.session_state:
    st.session_state.file_hash = None
if 'file_uploaded' not in st.session_state:
    st.session_state.file_uploaded = False

# ─── FILE UPLOADER DENGAN CACHE CLEAR ───────────────────────────────────────
with st.expander("📂 Import dari Excel — Modal_GGAC_v3.xlsx", expanded=False):
    st.markdown(
        ...
        "modal pengelola di sidebar. **PENTING**: Setelah edit Excel, upload ulang file "
        "untuk update data."
    )
    
    uploaded = st.file_uploader(
        "Pilih file Modal_GGAC_v3.xlsx", 
        type=["xlsx"],
        key="modal_upload", 
    ...
    if uploaded is not None:
        # Hitung hash file untuk deteksi perubahan
        file_content = uploaded.getvalue()
        current_hash = hashlib.md5(file_content).hexdigest()
        
        # Cek apakah file berbeda dari yang sebelumnya
        if st.session_state.file_hash != current_hash:
            try:
                with st.spinner("📖 Membaca file Excel..."):
                    # Parse ulang file
                    st.session_state.xl_data = parse_modal_excel(file_content)
                    st.session_state.file_hash = current_hash
                    st.session_state.file_uploaded = True
                    
                st.success("✅ File berhasil dibaca! Data sudah diupdate.")
                st.rerun()
            ...
            # File sama dengan yang sebelumnya
            if st.session_state.file_uploaded:
                st.info("ℹ️ File sudah dimuat. Edit Excel dan upload ulang untuk update.")
    
    # Tombol clear
    if st.session_state.file_uploaded:
        if st.button("🗑️ Hapus data Excel & kembali ke manual", key="clear_excel"):
            st.session_state.xl_data = {}
            st.session_state.file_hash = None
            st.session_state.file_uploaded = False
            st.rerun()
...
xl_data = st.session_state.xl_data
...
with st.sidebar:
    ...
    investasi = st.number_input("Investasi owner / Pihak I (Rp)",
                                value=xl_data.get("investasi", 101_553_000) if xl_data else 101_553_000,
    ...
    modal_pengelola = st.number_input("Modal pengelola / Pihak II — nilai buku alat (Rp)",
                                      value=xl_data.get("modal_pengelola", 8_757_600) if xl_data else 8_757_600,
