...
# ─── PARSER EXCEL MODAL_GGAC — VERSI DIPERBAIKI ──────────────────────────────
def parse_modal_excel(file_bytes):
    """
    Baca Modal_GGAC_v3 — scan dinamis dengan deteksi section yang ROBUST.
    Perbaikan utama: deteksi header fase lebih fleksibel & tracking section yang benar.
    """
    ...
    # ── Sheet 2: Investasi Investor — DETEKSI LEBIH ROBUST ───────────────────
    if "Investasi Investor" in wb.sheetnames:
        ws2 = wb["Investasi Investor"]
        
        fase1_gym, fase2_gym, pend_items = [], [], []
        section = "fase1"  # Default mulai dari fase 1
        buf_pct = 0.10
        
        # DEBUG: Print untuk troubleshooting
        print("=== DEBUG: Parsing Excel Investment ===")
        
        for row_idx, row in enumerate(ws2.iter_rows(), 1):
            # Gabungkan semua cell di row untuk deteksi header
            row_text = " ".join([str(cell.value or "") for cell in row]).upper()
            
            # Deteksi section header dengan lebih fleksibel
            if "FASE 1" in row_text and "PRIORITAS" in row_text and "SUBTOTAL" not in row_text:
                section = "fase1"
                print(f"Row {row_idx}: Detected FASE 1 section")
                continue
            elif "FASE 2" in row_text and "PENGEMBANGAN" in row_text and "SUBTOTAL" not in row_text:
                section = "fase2"
                print(f"Row {row_idx}: Detected FASE 2 section")
                continue
            elif "PENUNJANG" in row_text or "FASILITAS PENUNJANG" in row_text:
                section = "pend"
                print(f"Row {row_idx}: Detected PENUNJANG section")
                continue
            
            # Lewati baris subtotal/total/header
            if any(kw in row_text for kw in ["SUBTOTAL", "TOTAL", "NO.", "NAMA", "ITEM", "⚡", "HALAMAN", "RINGKASAN"]):
                continue
            
            # Buffer % detection
            if "BUFFER" in row_text:
                for ci in [2, 3, 4]:
                    v = row[ci].value if ci < len(row) else None
                    if isinstance(v, float) and 0 < v <= 1:
                        buf_pct = float(v)
                        print(f"Row {row_idx}: Found buffer {buf_pct*100}%")
                        break
                continue
            
            # Baris item: col A harus integer DAN col B harus ada nama
            if not isinstance(row[0].value, int):
                continue
            ...
            print(f"Row {row_idx}: Item '{nama[:30]}...' Qty={q} → Section={section}")
            
            if section == "fase1":
                fase1_gym.append(entry)
            elif section == "fase2":
                fase2_gym.append(entry)
            elif section == "pend":
                pend_items.append(entry)
        
        print(f"=== DEBUG: Total items - Fase1: {len(fase1_gym)}, Fase2: {len(fase2_gym)}, Penunjang: {len(pend_items)} ===")
