...
# ── PARSER EXCEL MODAL_GGAC (DIPERBAIKI: BACA DARI RINGKASAN) ──────────────
def parse_modal_excel(file_bytes):
    ...
    if "Investasi Investor" in wb.sheetnames:
        ...
        # ── PERBAIKAN: Membaca dari bagian RINGKASAN PERHITUNGAN (Baris 31-36) ──
        # Ini jauh lebih akurat daripada membaca Subtotal di baris 14/18/28
        
        gym_f1   = val("B31")  # Peralatan gym Fase 1
        gym_f2   = val("C32")  # Peralatan gym Fase 2 (sel C karena kolom B adalah 0/-)
        ...
        # Membaca item detail untuk tabel
        ...
        fase1_gym  = read_items(9,  13)  # Row 9-13 items
        fase2_gym  = read_items(16, 21) # Row 16-21 items
        pend_items = read_items(24, 29) # Row 24-29 items
...
# ── FILE UPLOADER (PERBAIKAN TAMPILAN BREAKDOWN) ───────────────────────────
with st.expander("📂 Import dari Excel — Modal_GGAC_v3.xlsx", expanded=False):
    ...
    if uploaded is not None:
        try:
            ...
            # ── Ambil nilai dari Excel (Menggunakan data yang sudah benar dari Ringkasan) ──
